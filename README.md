# Проектная работа "Веб-ларек"

Стек: HTML, SCSS, TS, Webpack

Структура проекта:
- src/ — исходные файлы проекта
- src/components/ — папка с JS компонентами
- src/components/base/ — папка с базовым кодом

Важные файлы:
- src/pages/index.html — HTML-файл главной страницы
- src/types/index.ts — файл с типами
- src/index.ts — точка входа приложения
- src/scss/styles.scss — корневой файл стилей
- src/utils/constants.ts — файл с константами
- src/utils/utils.ts — файл с утилитами

## Установка и запуск
Для установки и запуска проекта необходимо выполнить команды

```
npm install
npm run start
```

или

```
yarn
yarn start
```
## Сборка
```
npm run build
```

или

```
yarn build
```
## Архитектура проекта
Проект выполнен по MVP паттерну

## Описание данных
```
// Статусы продукта
export enum CART_ITEM_STATUS {
	NOT_IN_CART = 0,
	IN_CART = 1,
	NO_PRICE = 2,
}
//Методы оплаты
export enum PAYMENT_METHOD {
	ONLINE = 'Онлайн',
	RECEIVED = 'При получении',
}

// Информация о продукте
interface IProductItem {
  id: string; // Идентификатор
  description: string; // Описание
  image: string; // Эндпоинт картинки
  title: string; // Название
  category: string; // Категория
  price: number | null // Цена
}
// Статус продукта
export interface ICartStatus {
	cartStatus: CART_ITEM_STATUS;
}
// Продукт
export type IProduct = IProductItem & ICartStatus;

// Приложение
interface IAppState {
  catalog: IProductItem[]; // Каталог товаров
  cart: string[]; // Корзина
}
//Форма заказа
export interface IOrderForm {
	address: string; // Адрес
	email: string; // Email
	phone: string; // Телефон
}
// Заказ
export interface IOrder extends IOrderForm {
	payment: PAYMENT_METHOD; // Способ оплаты
	total: number; // Стоимость товаров в заказе
	items: string[]; // Товары в заказе
}

export type FormErrors = Partial<Record<keyof IOrder, string>>; // Ошибки формы

export interface IOrderResult {
	id: string; // Идентификатор заказа
	total: number; // Стоимость товаров в заказе
}

```

## Базовый код
### 1. Класс EventEmitter
Реализует паттерн «Наблюдатель» и позволяет подписываться на события и уведомлять подписчиков
о наступлении события.
Класс имеет методы ```on```, ```off```, ```emit``` — для подписки на событие, отписки от события и уведомления
подписчиков о наступлении события соответственно.

### 2. Класс Component
Базовый компонент предоставляющий инструменты для работы с DOM дочерних классов.
Класс имеет методы: 

```setText``` - Устанавливает текстовое содержимое.  
```setImage``` - Устанавливает изображение с альтернативным текстом.  
```setDisabled``` - Запретить взаимодействие с элементом.  
```render``` - Возвращает корневой элемент.  

## Компоненты модели данных
### 1. Класс AppData
Управляет приложением. Реализует тип IAppState.
Класс хранит данные о каталоге и товарах корзины, а так же данные о заказе и предоставляет методы для работы с ними, такие как:

```setCatalog``` - Получает каталог и сохраняет его.  
```getCatalog``` - Возвращает каталог.  
```updateCatalog``` - Обновляет статусы товаров в каталоге.  
```setActiveCardId``` - Сохраняет идентификатор карточки товара.  
```setActiveCardId``` - Возвращает идентификатор карточки товара.  
```addToCart``` - Добавляет товар в корзину.  
```deleteFromCart``` - Удаляет товар из корзины.  
```clearCart``` - Очищает корзину.  
```getTotal``` - Возвращает стоимость всех товаров в корзине.  
```getCartProductList``` - Получает список товаров в корзине.  
```getCartCount``` - Возвращает количество товаров в корзине.  
```setPaymentMethod``` - Сохраняет метод оплаты.  
```setPaymentMethod``` - Возвращает метод оплаты.  
```setOrderCart``` - Сохраняет текущую корзину в заказ.  
```setOrderTotal``` - Сохраняет текущую стоимость товаров в корзине в заказ.  
```setOrderField``` - Получает и сохраняет данные формы.  
```validateOrder``` - Валидирует данные формы.  

### 2. Класс ShopAPI
Позволяет взаимодействовать с сервером.

Класс имеет методы:  
```getProductList``` - Получает список товаров с сервера.  
```getProduct``` - Получает товар с сервера.  
```orderProducts``` - Передает заказ серверу.  

## Компоненты представления
### 1. Класс Modal
Отображает модальное окно.
Реализует модальное окно в котором может отображаться какое либо содержимое ```(content: HTMLElement)```. 
Так же предоставляет методы ```open``` и ```close``` для управления отображением модального окна.

### 2. Класс Card
Отображает карточку товара.

Карточка формируется на основании получаемого темплейта и отвечает за взаимодействие пользователя с карточкой генерируя событие ```cardModal:open``` и ```card:select``` при клике на ней (для каталога товаров на главной) или обрабатывает клик на кнопке внутри модального окна товара ```cartItem:add``` или ```cartItem:delete``` (событие происходит в зависимости от наличия товара в корзине).

### 3. Класс Cart
Отображает корзину.

```
interface ICartView {
    items: HTMLElement[]; // Список товаров в корзине
    total: number; // Общая стоимость товаров в корзине
    length: number; // Кол-во товаров в корзине
}
```

Корзина формируется на основании получаемого темплейта и отвечает за взаимодействие пользователя с корзиной генерируя событие ```details:open`` при клике на кнопку "Оформить".

### 4. Класс Form
Отображает форму оформления заказа.

```
interface IFormState {
    valid: boolean; // Валидность формы
    errors: string[]; // Ошибки формы
}
```

Форма формируется на основании получаемого темплейта и отвечает за взаимодействие пользователя с формой.
При переходе на второй шаг формы (по кнопке "Далее") генерируется событие ```details:submit```.  
При клике по кнопке "Оплатить" генерируется событие ```contacts:submit```

### 5. Класс Success
Отображает подтверждение заказа.

```
interface ISuccess {
    total: number; // Стоимость товаров в оформленном заказе.
}
```

### 6. Класс Page
Отображает главную страницу.

```
interface IPage {
  counter: number; // Счетчик товаров в корзине
  catalog: HTMLElement[]; // Каталог товаров
}
```
При клике на иконку корзины, генерируется событие ```cart:open```.

## Взаимодействие между компонентами
Класс ```ShopApi``` получает данные с сервера. После класс ```AppData``` собирает из полученных данных массив карточек отправляет его на отрисовку в класс ```Page```, карточка каждого товара формируется в классе ```Card```. Все модальные окна отрисовываются с помощью класса ```Modal```. При клике на карточку по событию ```card:select``` определяется какая карточка была выбрана и показывается её модальное окно. При клике на кнопку "В корзину", с помощью метода класса ```AppData```, товар добавляется в корзину. При клике на иконку корзины с помощью класса ```Cart``` происходит отображение модального окна корзины, данные о корзине получаются из класса ```AppData```. При клике кнопки "Оформить" показывается модальное окно формы с помощью класса ```Form```. Данные из формы отправляются в класс ```AppData```, где проходят валидацию, при успешной валидации данных формы с помощью класса ```ShopApi```, заказ отправляется на сервер, если же данные формы не проходят валидацию, то в консоли появляется сообщение об ошибке. При положительном ответе от сервера с помощью класса ```Success``` отображается подтверждение заказа.

Все взаимодействия между слоями данных и отображения происходят в файле index.ts, находящийся в корне проекта.